...
public class TastyNastyService {
    ...
    public String last5MinuteResult(Long id){
        ...
        ArrayList<Tasty> dishTastyVotes =(ArrayList)tasties.findAllByTheDish_IdAndVoteAtAfter(id, LocalDateTime.now().minusMinutes(5));
        System.out.println("Tasty Count:" + dishTastyVotes.size());

        ArrayList<Nasty> dishNastyVotes =(ArrayList)nasties.findAllByTheDish_IdAndVoteAtAfter(id, LocalDateTime.now().minusMinutes(5));
        System.out.println("Nasty Count:" + dishNastyVotes.size());

        if(dishNastyVotes.size()>dishNastyVotes.size())
        { tastyOrNasty="tasty";
        }
